...
public class Gun : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (mFiring && fireEnabled)
        {
            ...
            if (shotgun)
            {
                secondsPerBullet = 10.0f / (fireRate / 60.0f);
            }
            else
            {
    ...
    public void ShootGun(Transform director)
    {
        ...
        {   //using shotgun
            //AdjustFireRate(1.0f);
            //AdjustSpreadSize(shotgunBullets);
            for (int i = 0; i < shotgunBullets; i++)
            {   //spawn more bullets
                float bulletOffset = shotgunSpread - i * 3;
                SpawnBullet(
                    director.position,
                    Quaternion.Euler(director.eulerAngles.x, director.eulerAngles.y + bulletOffset, director.eulerAngles.z)
                );
            }
